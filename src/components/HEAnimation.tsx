    ...
    <div className="flex flex-col items-center justify-center bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-5xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div className="relative h-[300px] md:h-[500px] bg-slate-900 overflow-hidden flex items-center justify-center">
          ...
          {/* Connection Line - Absolutely positioned between box centers */}
          {/* Mobile: px-4 (1rem) + box center (w-16/2 = 2rem) = 3rem from each side */}
          {/* Line should be at box center vertically: 50% - half box height (h-16/2 = 2rem) */}
          <div 
            id="connection-line-mobile"
            className="absolute md:hidden h-[2px] bg-slate-700 z-0 pointer-events-none"
            style={{
              left: 'calc(1rem + 2rem)', // px-4 + half box width
              right: 'calc(1rem + 2rem)', // px-4 + half box width  
              top: 'calc(50% - 2rem)', // 50% - half box height (h-16 = 4rem, so half is 2rem)
            }}
          />
          {/* Desktop: px-24 (6rem) + box center (w-32/2 = 4rem) = 10rem from each side */}
          {/* Line should be at box center vertically: 50% - half box height (h-32/2 = 4rem) */}
          <div 
            id="connection-line-desktop"
            className="absolute hidden md:block h-[2px] bg-slate-700 z-0 pointer-events-none"
            style={{
              left: 'calc(6rem + 4rem)', // px-24 + half box width
              right: 'calc(6rem + 4rem)', // px-24 + half box width
              top: 'calc(50% - 4rem)', // 50% - half box height (h-32 = 8rem, so half is 4rem)
            }}
          />

          <div className="relative z-10 w-full flex justify-between px-4 md:px-24 items-center">
            ...
            <div className="flex flex-col items-center relative z-10">
              ...
              {/* Local Data Object - Fixed position container */}
              <div className="h-16 md:h-24 mt-3 md:mt-6 flex items-start justify-center relative">
            ...
            {/* Traveling Packets - Positioned relative to connection line */}
            {/* Mobile container matches line position */}
            <div 
              className="absolute md:hidden pointer-events-none z-5"
              style={{
                left: 'calc(1rem + 4rem)', // Right edge of client box
                right: 'calc(1rem + 4rem)', // Left edge of server box
                top: 'calc(50% - 2rem)', // Align with line (box center)
                height: '2px',
              }}
            >
              {step === 1 && (
                <motion.div
                  key="packet-to-server-mobile"
                  ...
                  className="absolute bg-blue-500 p-1 rounded-lg shadow-lg flex items-center gap-1"
                  style={{ top: '-50%', transform: 'translate(-50%, -50%)' }}
                >
                  <Lock size={10} className="text-white" />
                  <div className="h-0.5 w-4 bg-white/30 rounded" />
                </motion.div>
              )}
              {step === 3 && (
                <motion.div
                  key="packet-to-client-mobile"
                  ...
                  className="absolute bg-purple-500 p-1 rounded-lg shadow-lg flex items-center gap-1"
                  style={{ top: '-50%', transform: 'translate(-50%, -50%)' }}
                >
                  <div className="h-0.5 w-4 bg-white/30 rounded" />
                  <Lock size={10} className="text-white" />
            ...
            {/* Desktop container matches line position */}
            <div 
              className="absolute hidden md:block pointer-events-none z-5"
              style={{
                left: 'calc(6rem + 8rem)', // Right edge of client box
                right: 'calc(6rem + 8rem)', // Left edge of server box
                top: 'calc(50% - 4rem)', // Align with line (box center)
                height: '2px',
              }}
            >
              {step === 1 && (
                <motion.div
                  key="packet-to-server-desktop"
                  initial={{ left: '0%', opacity: 0 }}
                  animate={{ 
                    left: '100%',
                    opacity: [0, 1, 1, 0],
                  ...
                  className="absolute bg-blue-500 p-1 md:p-2 rounded md:rounded-lg shadow-lg flex items-center gap-1 md:gap-2"
                  style={{ top: '-50%', transform: 'translate(-50%, -50%)' }}
                >
              ...
              {step === 3 && (
                <motion.div
                  ...
                  className="absolute bg-purple-500 p-1 md:p-2 rounded md:rounded-lg shadow-lg flex items-center gap-1 md:gap-2"
                  style={{ top: '-50%', transform: 'translate(-50%, -50%)' }}
                >
                  <div className="h-0.5 md:h-1 w-4 md:w-8 bg-white/30 rounded" />
                  <Lock size={10} className="md:w-3.5 md:h-3.5 text-white" />
            ...
            <div className="flex flex-col items-center relative z-10">
              ...
              {/* Server Processing Object - Fixed position container */}
              <div className="h-16 md:h-24 mt-3 md:mt-6 flex items-start justify-center relative">
                <AnimatePresence mode="wait">
                  {step === 2 && (
                    <motion.div 
